...
接下来继续介绍下 epoll 模式，epoll 可以说是 Linux 下多路复用 IO 接口 select和poll 的增强版本，在高并发连接中，它能显著提高程序在只有少量活跃情况下系统的 CPU 利用率，因为它会复用文件描述符集合来传递结果，而不需要每次都等待事件的到来才进行重新准备要被监听的文件描述符集合；另外，由于获取事件的时候，它无须遍历整个被监听的描述符集，只要遍历那些被内核 IO 事件异步唤醒而加入 Ready 队列的描述符集合即可。

目前 epoll 模式是 linux 大规模并发网络程序中使用最多的一种。epoll 除了提供 IO 事件的电平触发外，还提供了边沿触发，这就使得用户空间程序有可能缓存 IO 状态，减少 epoll_pwait 的调用，即减少等待事件的处理，提高应用程序效率。

我们可以使用 cat 命令查看一个进程可以打开的 socket 描述符的上限，在命令行执行 `cat /proc/sys/fs/file-max`，结果如下图所示：
