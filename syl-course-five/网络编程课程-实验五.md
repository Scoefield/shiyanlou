...
服务器按实现方式可以分为迭代类和并发类服务。我们平时用 C 语言编写的简单 Socket 服务端和客户端通信是属于迭代类的，即服务器每次只能处理一个客户端的请求，其特点是实现起来比较简单，但是处理效率比较低。

而在实际应用中，并发类服务用的比较多，通常不会让一个服务长时间处理一个客户端请求，而是需要服务端可以同时处理多个客户请求的能力，这种可以同时处理多个客户端请求的服务器，我们一般称为并发服务器，并发服务的效率高，但实现起来有点复杂。

linux 下通常有三种实现并发服务器的方式：多进程并发服务器，多线程并发服务器和IO 多路复用。

先来看多进程并发服务器的实现。实现多进程并发服务器时，我们需要考虑以下几点：

1. 父进程创建套接字（文件描述符）上限数量（父进程中需要 close 关闭 accept 返回的新文件描述符）
2. 系统内允许创建的进程个数（一般与内存大小有关）
3. 进程创建过多后是否会降低服务性能（进程调度以及资源消耗情况）
